...
\author[1]{Di Wang}
\author[1]{Jian Cui}
\author[1]{Qiu-Ye Yu}\cormark[1]
\ead{yuqiuye@jisu.edu.cn}

% Affiliation
\affiliation[1]{organization={School of AI, Jilin International Sciences University},
            city={Changchun},
            country={China}}
...
% Abstract
\begin{abstract}
Learning rate scheduling has undergone a remarkable evolution from the single global fixed rate of early SGD to sophisticated layer-wise adaptive strategies. This research systematize this evolution into five generations: (Gen1) global fixed, (Gen2) global scheduling, (Gen3) parameter-level adaptation, (Gen4) layer-level differentiation, and (Gen5) joint layer-time scheduling. We trace the fundamental motivation behind each transition, showing how the shift from ``one-size-fits-all'' to ``tailoring by layer and time'' addresses the impossible trinity of transfer learning: lower layers require small updates to preserve general knowledge while higher layers need large updates to adapt to new tasks. Building on this taxonomy, we propose Discriminative Adaptive Layer Scaling (DALS), a unified framework integrating phase-adaptive cosine scheduling, depth-aware Grokfast gradient filtering, and LARS-style trust ratios into a single coherent optimizer. We benchmark 18 strategies including three DALS variants across all five generations on five datasets: synthetic (MLP), CIFAR-10 (ConvNet), RTE TREC-6, and IMDb (DistilBERT, fine-tune). On synthetic, DALS achieves the best accuracy at 98.0\%, while DALS-Fast reaches 90\% in just 3 epochs. The cross-dataset analysis reveals striking regime-dependent patterns: strategies that dominate from-scratch training (DALS, Fixed SGD) falter on fine-tuning tasks, while adaptive methods (RAdam, Lookahead) excel on benchmarks---no single strategy wins across all regimes. Critically, STLR+Discriminative---the ULMFiT---fails on from-scratch tasks (43.6\%), confirming that directional decay biases are harmful without pretrained features. Conversely, on IMDb fine-tuning, discriminative methods recover to competitive levels. phase-and-depth aware design of DALS avoids either extreme, achieving the best synthetic result while maintaining competitive fine-tuning performance.
\end{abstract}

% Research highlights
\begin{highlights}
\item A five-generation taxonomy traces learning rate evolution from global fixed to joint layer-time scheduling, systematizing nearly four decades of optimization research.
\item DALS --- a unified optimizer combining phase-adaptive cosine scheduling, depth-aware Grokfast gradient filtering, and LARS-style trust ratio scaling --- with speed and accuracy variants.
\item Cross-dataset benchmark of 18 strategies across five datasets reveals regime-dependent dominance: no single strategy wins universally across from-scratch and fine-tuning tasks.
\item STLR+Discriminative (the ULMFiT champion) suffers catastrophic failure on from-scratch training (43.6\%), confirming directional decay biases are harmful without pretrained features.
\end{highlights}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{paper_fig1_impossibility}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{paper_fig2_taxonomy}
\caption{Five-generation taxonomy of learning rate strategies. The evolution progresses from global fixed LR (Gen1) \texorpdfstring{$\to$}{->} global scheduling (Gen2) \texorpdfstring{$\to$}{->} parameter-level adaptation (Gen3) \texorpdfstring{$\to$}{->} layer-level differentiation (Gen4) \texorpdfstring{$\to$}{->} joint layer\texorpdfstring{$\times$}{x}time scheduling (Gen5). DALS (SOTA) integrates all five generations.}\label{fig:taxonomy}
\end{figure*}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{paper_fig3_discriminative}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{paper_fig4_stlr}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{paper_fig5_dals_architecture}
...
\begin{table*}[width=.6\textwidth,cols=3,pos=h]
...
Each component has been independently validated; DALS provides a principled framework for combining them with phase-aware and depth-aware coordination. Notably, DALS removes the directional bias of discriminative decay (Section~\ref{sec:related}) that suppresses lower-layer updates---a bias designed for transfer learning but harmful for from-scratch training.
...
\begin{table*}[width=\textwidth,cols=7,pos=h]
\caption{Cross-dataset benchmark---best test accuracy (\%) of 18 strategies on 5 datasets. Bold indicates best per column.}\label{tab:benchmark}
\small
\begin{tabular*}{\tblwidth}{@{} L C C C C C C @{}}
\toprule
...
\begin{table*}[width=.55\textwidth,cols=5,pos=h]
