"""
SST-2 benchmark: DistilBERT fine-tuning with 18 strategies.
Validates transfer learning scenario (pretrained model + NLP task).
MPS/CUDA acceleration, mixed precision.
"""
...
print(f"Device: {DEVICE} | AMP: {USE_AMP} | Threads: {torch.get_num_threads()}")
...
def create_optimizer(name, model, total_steps):
    is_sam = False
    scheduler = None
    steps_per_epoch = 67349 // BATCH_SIZE + 1
    ...
    if name == 'Gen1_FixedSGD':
        ...
        opt = SGDRWithRestarts(model.parameters(), lr=LR_SGD * 2, T_0=steps_per_epoch, T_mult=2, momentum=0.9, weight_decay=0.01)
    elif name == 'Gen3_Adam':
        ...
        opt = DALSAcc(wrapper, lr=LR_SGD, momentum=0.9, weight_decay=0.01,
                      ...
                      T_0=steps_per_epoch * 2, T_mult=2)
    else:
...
def train_and_evaluate(name, epochs=EPOCHS):
    ...
    print(f"  Loading SST-2 dataset...", flush=True)
    ...
    for ep in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.no_grad():
                preds = out.logits.argmax(dim=-1) if not is_sam else model(input_ids=input_ids, attention_mask=attention_mask).logits.argmax(dim=-1)
        ...
        lr_now = opt.param_groups[0].get('lr', LR_BASE)
        t_now = time.time() - t0

        for thr in [85, 88, 90, 91, 92]:
        ...
        print(f"  Ep {ep}/{epochs}: train={train_acc:.1f}%  val={test_acc:.1f}%  lr={lr_now:.2e}  t={t_now:.1f}s", flush=True)
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*90}")
    print(f"  SST-2 BENCHMARK (DistilBERT) — {len(strategies)} strategies, {EPOCHS} epochs, device={DEVICE}")
    print(f"{'='*90}\n")

    all_results = {}
    checkpoint_file = RESULTS_DIR / 'sst2_results.json'
    ...
    with open(RESULTS_DIR / 'sst2_results.json', 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"\nSaved to {RESULTS_DIR / 'sst2_results.json'}")